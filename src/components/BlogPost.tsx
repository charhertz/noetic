...
import { NotionPost } from '@/lib/notion/type'

const BlogPost = ({ post }: { post: NotionPost }) => {
  return (
    <motion.div>
      <Link passHref href={`${config.siteUrl}/post/${post.slug}`} scroll={false}>
        <article
          key={post.id}
        ...
        >
          ...
          <div className='relative mt-auto'>
            ...
            <p className='font-light hidden md:block leading-8 text-gray-700 dark:text-gray-300'>{post.description}</p>
            {/* w-4/5  */}
          </div>
        </article>
      </Link>
